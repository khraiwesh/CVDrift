...
# Single-approach result tables (like CVDrift outputs) have no tunable params.
used_parameters["CVDrift"] = ["Algorithm"]
...
def evaluate(csv_path="algorithm_results.csv", out_path="Evaluation_Results", lag_window:int=200, min_support:int=1, verbose:bool = False):
    ...
    # Evaluate directly on the provided results table (no preprocess filtering).
    ...
    for col in ("Detected Changepoints", "Actual Changepoints for Log"):
        if col in df.columns:
            df[col] = df[col].map(_parse_int_list)

    if pd.read_csv(csv_path).duplicated().any():
        raise ValueError("CSV contains duplicate rows!")

    # Ensure the expected algorithm metadata exists for single-approach CSVs.
    if "Algorithm" not in df.columns:
        df["Algorithm"] = "CVDrift"

    unknown = [name for name in df["Algorithm"].unique() if name not in used_parameters]
    if unknown:
        raise ValueError(
            f"Algorithm(s) missing from used_parameters: {unknown}. "
            "Add their grouping columns in used_parameters first."
        )

    alpha_sort_names = sorted(list(df["Algorithm"].unique()))
    colors = ["#DB444B", "#9A607F", "#006BA2", "#3EBCD2", "#379A8B", "#EBB434", "#B4BA39", "#D1B07C"]

    # Use the exact Experiment#2 evaluator to keep scores identical to run_experiments.py.
    run_exp_path = Path(__file__).with_name("run_experiments.py")
    spec = importlib.util.spec_from_file_location("run_experiments_eval", run_exp_path)
    run_exp = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(run_exp)
    summary = run_exp.evaluate_results_csv(csv_path, tol=lag_window)

    # Keep the existing plot path, but for a single-approach results table only.
    computed_accuracy_dicts = {name: {name: summary["F1"]} for name in alpha_sort_names}
    computed_precision_dicts = {name: {name: summary["Precision"]} for name in alpha_sort_names}
    computed_recall_dicts = {name: {name: summary["Recall"]} for name in alpha_sort_names}
    accuracy_best_param = {name: name for name in alpha_sort_names}
    plot_accuracy(
        computed_accuracy_dicts,
        computed_precision_dicts,
        computed_recall_dicts,
        accuracy_best_param,
        out_path,
        lag_window,
        colors,
    ...
    results_df = pd.DataFrame(
        [
            ...
            for name in alpha_sort_names
        ]
    )
    results_df.to_csv(f"{out_path}/evaluation_accuracy_only_{lag_window}.csv", index=False)
...
def calcAccuracy(df:pd.DataFrame, param_names:List[str], lag_window: int):
    ...
    def _tp_fp_one_to_one(detected_cp, actual_cp, tolerance):
        """Match run_experiments.py behavior: closest one-to-one TP matching."""
        detected_sorted = sorted(int(x) for x in detected_cp)
        gt_sorted = sorted(int(x) for x in actual_cp)
        ...
        for gt in gt_sorted:
            ...
            for index, detected in enumerate(detected_sorted):
                if index in matched:
                    continue
                distance = abs(detected - gt)
        ...
        fp_local = len(detected_sorted) - tp_local
        return tp_local, fp_local
    ...
    for parameters, group in df.groupby(by=param_names):
        ...
        for index, row in group.iterrows():
            ...
            tp, fp = _tp_fp_one_to_one(detected_cp, actual_cp, lag_window)
            tps += tp
            fps += fp
